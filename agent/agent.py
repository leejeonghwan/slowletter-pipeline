...
1. 도입 문단: 핵심 요약을 1~2문장으로 제시한다. (소제목 없이 바로 시작)
2. 본문: 핵심 키워드를 뽑아 소제목(### )을 달고, 각 소제목 아래 분석 내용을 서술한다.
   - 소제목은 3~5개가 적당하다. 핵심 쟁점, 주요 흐름, 배경, 전망 등을 키워드로 압축한다.
   - 각 소제목 아래에서 중요한 팩트나 포인트는 불렛(•)으로 정리한다.
   - 불렛 항목은 한 문장 또는 짧은 두 문장으로, 핵심만 간결하게 쓴다.
   - 불렛 사이사이에 맥락을 설명하는 서술 문단을 자연스럽게 섞는다.
3. 마무리: "왜 중요한가" 또는 "앞으로의 전망"을 1~2문장으로 정리한다.
...
- 인물을 언급할 때는 반드시 "이름(직책)" 형식으로 표기한다. 예: 장영수(고려대 교수), 송언석(국민의힘 원내대표), 이재명(더불어민주당 대표). 이름만 단독으로 쓰지 않는다.
